...
Verified Module. It is allowed, under Naming below, for a generated name. The
...
Two length exceptions to know, and they are not the same exception. Storage
accounts take no hyphens and cap at 24 characters, so they compress to
`st<purpose><env><loc><instance>`. Key vaults cap at 24 characters too but do
allow hyphens, so they keep the normal component order,
`kv-<purpose>-<env>-<loc>[-<instance>]`; only the length needs watching, plus
starting with a letter and no consecutive hyphens. Resource types with a tight
limit need checking against
...
TG_NO_AUTO_INIT=true \
  terragrunt --working-dir live/<unit> validate          # only after the init above
...
- [ ] `init -backend=false` then the offline `validate` pass for every changed unit
