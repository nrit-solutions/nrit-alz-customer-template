...
#
# To upgrade the catalog, bump catalog_version and open a pull request: the plan
...
stack "caf_platform_foundation" {
  source = "${local.catalog_url}//stacks/caf-platform-foundation?ref=${local.catalog_version}"
  ...
  values = {
    ...
    # policy default values from them), so set them once at the top level.
    ...
    # ALZ hierarchy and policy base library, shared by the landing-zones and
    # amba-policy units so both read the same base policy definitions.
    alz_library_ref = "2026.04.2"

    # Azure Monitor Baseline Alerts (AMBA). Mandatory in the NRIT baseline (ADR
    # 13). REQUIRED: replace the placeholder with a real monitored inbox during
    # onboarding (see ONBOARDING.md, step 2); all AMBA alerts route to this
    # action group. The names follow the region-suffixed convention.
    ...
    # Management subscription resource settings live in the management namespace.
    management = {
      log_analytics_workspace_retention_in_days = 30
    }

    # Management group hierarchy, policy, and subscription placement live in the
    # landing-zones namespace.
    landing_zones = {
      # ALZ hierarchy and policy from the public ALZ Library, layered with the
      # nrit custom library that adds tag governance (Enforce-Tag-Gov denies RGs
      # missing the four mandatory tags). architecture_name = "alz" +
      # custom_library_url = "" falls back to stock ALZ with no tag governance.
      architecture_name  = "nrit"
      custom_library_url = "${local.catalog_url}//library/platform/nrit?ref=${local.catalog_version}"
